                      ...
                      bool showNameChangeConfirm, bool showDeleteDataConfirm)
{
    ...
    // Name-Ändern Bestätigungsdialog
    if (showNameChangeConfirm)
    {
        ...
        DrawRectangleRec(yBtn, CheckCollisionPointRec(mousePoint, yBtn) ? GOLD : DARKGRAY);
        DrawRectangleRec(nBtn, CheckCollisionPointRec(mousePoint, nBtn) ? GRAY : DARKGRAY);
        DrawRectangleLinesEx(yBtn, 2, GOLD);
        DrawRectangleLinesEx(nBtn, 2, {160, 160, 160, 200});
        int yw = MeasureText(isEnglish ? "YES" : "JA", 22);
        DrawText(isEnglish ? "YES" : "JA", (int)(yBtn.x + (yBtn.width - yw) / 2), (int)(yBtn.y + 14), 22, WHITE);
        int nw = MeasureText(isEnglish ? "NO" : "NEIN", 22);
        DrawText(isEnglish ? "NO" : "NEIN", (int)(nBtn.x + (nBtn.width - nw) / 2), (int)(nBtn.y + 14), 22, WHITE);
    }

    // Daten-Löschen Bestätigungsdialog
    if (showDeleteDataConfirm)
    {
        ...
        Rectangle yBtn = {290, 460, 150, 50};
        Rectangle nBtn = {480, 460, 150, 50};
        DrawRectangleRec(yBtn, CheckCollisionPointRec(mousePoint, yBtn) ? RED : MAROON);
        DrawRectangleRec(nBtn, CheckCollisionPointRec(mousePoint, nBtn) ? GRAY : DARKGRAY);
        DrawRectangleLinesEx(yBtn, 2, {255, 80, 80, 200});
        DrawRectangleLinesEx(nBtn, 2, {160, 160, 160, 200});
        int yw = MeasureText(isEnglish ? "YES, DELETE" : "JA, LOESCHEN", 20);
        DrawText(isEnglish ? "YES, DELETE" : "JA, LOESCHEN", (int)(yBtn.x + (yBtn.width - yw) / 2), (int)(yBtn.y + 15), 20, WHITE);
        int nw = MeasureText(isEnglish ? "NO" : "NEIN", 22);
        DrawText(isEnglish ? "NO" : "NEIN", (int)(nBtn.x + (nBtn.width - nw) / 2), (int)(nBtn.y + 14), 22, WHITE);
    }
}
                  ...
                  bool isEnglish, Texture2D carTexs[])
{
    ...
    const char *blueLabel = data.ownsBlueCar
                                ? (isEnglish ? "SELECT" : "WAEHLEN")
                                : TextFormat("%d Stars", PRICE_BLUE_CAR);
...
void DrawDescriptionMenu(Vector2 mousePoint, Rectangle backBtn, bool isEnglish)
{
    ...
    DrawTextCentered(isEnglish ? "LEFT / RIGHT ARROWS  -  Move car"
                               : "LINKS / RECHTS Pfeile  -  Fahrzeug lenken",
                     162, 22, RAYWHITE);
    DrawTextCentered(isEnglish ? "ESC  -  Pause menu (resume, menu, volume, quit)"
                               : "ESC  -  Pause-Menue (Weiter, Menue, Lautstaerke, Beenden)",
                     194, 22, RAYWHITE);

    DrawTextCentered(isEnglish ? "-- POWER-UPS --" : "-- POWER-UPS --", 250, 22, secColor);
    DrawTextCentered(isEnglish ? "CLOCK  -  Slows the game for 3 seconds"
                               : "UHR  -  Verlangsamt das Spiel fuer 3 Sekunden",
                     282, 22, {100, 220, 255, 255});
                   ...
                   bool showQuitConfirm, bool showPauseBackConfirm)
{
    ...
    DrawText(volLabel, (1000 - labelW) / 2, 495, 20, RAYWHITE);
    ...
    // Zurück-zum-Menü Bestätigungsdialog
